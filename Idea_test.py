...
if __name__ == '__main__':
    ...
    model_sub = ResNet().to(device)
    # 定义损失函数loss，以及优化器
    loss_func=nn.CrossEntropyLoss()
    optimizer=torch.optim.Adam(model.parameters(),lr=LR)
    optimizer_sub = torch.optim.Adam(model_sub.parameters(), lr=LR)
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            out = model(images)
            out_sub = model_sub(images)
            optimizer.zero_grad()
            optimizer_sub.zero_grad()
            if i%2 == 0:
                out_con = out-out_sub
                loss = loss_func(out_con,labels)
                loss.backward()
            else:
                loss = loss_func(out, labels)
                loss.backward()
                loss_sub = loss_func(out_sub, labels)
                loss_sub.backward()
            optimizer.step()
            optimizer_sub.step()
            if (i + 1) % 100 == 0:
        ...
        with torch.no_grad():
            ...
            print("Accuracy of ResNet model in CIFAR-100 : {:4f}".format(correct / total * 100))
